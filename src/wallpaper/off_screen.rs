...
impl OffScreen {
    fn create_texture(device: &wgpu::Device, size: (u32, u32)) -> wgpu::Texture {
        ...
        device.create_texture(&texture::image_srgb_unorm_desc(
    ...
    fn update_texture(&mut self, device: &wgpu::Device, size: (u32, u32)) {
        ...
        let size = texture::texture_size(size.0, size.1);
        let texture = device.create_texture(&texture::image_srgb_unorm_desc(
            Some("Off-screen texture"),
            size,
            1,
        ));
    ...
    pub fn create(device: &wgpu::Device, size: (u32, u32)) -> Self {
        ...
        let render_pipeline = render_pipeline::create_pipeline(
            ...
            Some("fs_main"),
            &shaders::wallpaper::targets(wgpu::TextureFormat::Rgba8UnormSrgb),
        );
